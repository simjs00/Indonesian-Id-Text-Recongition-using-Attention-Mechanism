...
def extract_images(filename):
  ...
  with gzip.open(filename) as bytestream:
    ...
    res = rows * cols * num_images
    buf = bytestream.read(res[0])
...
def extract_images_labels(filename):
  """Extract the images into a 4D uint8 numpy array [index, y, x, depth]."""
  print('\nGet Supervised Data and Labels', filename)
  import cv2
  import glob
  import numpy as np
  num_images=0
  data= []
  labels=[]
  lbl = get_class(filename)
  for name in os.listdir(filename) :
    for name1 in os.listdir(filename+"/"+name)  :

      for i, files in enumerate (glob.glob("./"+filename+"/"+name+"/"+name1+"/*png")):
          flag=0
...
def extract_images_unsupervised(filename):
  """Extract the images into a 4D uint8 numpy array [index, y, x, depth]."""
  print('\nGet Unsupervised Train Data ', filename)
  PIK ='/pickle.dat'
  data = None
  with open(filename+PIK, "rb") as f:
      data= pickle.load(f)
      print("Load Uns Done", len(data))
...
def read_data_sets(train_dir, fake_data=False, one_hot=False):
  ...
  train_images,train_labels = extract_images_labels(local_file)
  ...
  # test_images,test_labels = extract_images_labels(local_file)
...
def read_data_sets_pretraining(train_dir):
  ...
  local_file = maybe_download(TRAIN_IMAGES, train_dir)
  train_images = extract_images_unsupervised(local_file)
...
def fill_feed_dict_1(img, images_pl, noise=False):

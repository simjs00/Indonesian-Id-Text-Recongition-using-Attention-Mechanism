...
from recognizer.src.data_util.data_gen import DataGen
...
class Model(object):
    ...
    def decode_text(self,output) :
           ...
           output=[chr(c-3+32)for c in chars]
           return output
    def test (self,img_list) :
        for img in img_list :

            self.s_gen.prepare_test_data(img)
            ...
            for batch in self.s_gen.gen(self.batch_size):
                ...
                step_outputs= self.decode_text(step_outputs)
                step_outputs= ''.join(step_outputs)
                sys.stdout.write(step_outputs +" ")
            print()
            #print(step_outputs)
            #return  step_outputs
    ...
    def launch(self):
        ...
        if self.phase == 'test':
            ...
            self.s_gen.gen(self.batch_size)
    ...
    def visualize_attention(self, filename, attentions, output_valid, ground_valid, flag_incorrect, real_len):
        ...
        with open(os.path.join(output_dir, 'word.txt'), 'w') as fword:
            ...
            fword.write(' '.join([chr(c-3+32)for c in ground_valid])+'\n')
            fword.write(' '.join([chr(c-3+32)for c in output_valid]))
